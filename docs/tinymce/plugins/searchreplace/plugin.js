...
var searchreplace = (function () {
  ...
  var $_bhcgtvisjew5etbz = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_bhcgtvisjew5etbz.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_rz8qxiqjew5etbq.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_rz8qxiqjew5etbq.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_rz8qxiqjew5etbq.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_rz8qxiqjew5etbq.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_rz8qxiqjew5etbq.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_rz8qxiqjew5etbq.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_rz8qxiqjew5etbq.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_rz8qxiqjew5etbq.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_rz8qxiqjew5etbq.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_rz8qxiqjew5etbq.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_rz8qxiqjew5etbq.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_rz8qxiqjew5etbq.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_rz8qxiqjew5etbq.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_rz8qxiqjew5etbq.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_rz8qxiqjew5etbq.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_7j0vtyiujew5etc4 = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_7j0vtyiujew5etc4.open(editor, currentIndexState);
    });
  };
  var $_appzsditjew5etc3 = { register: register };
  ...
  var $_5jbay2ivjew5etc7 = { register: register$1 };

  PluginManager.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_appzsditjew5etc3.register(editor, currentIndexState);
    $_5jbay2ivjew5etc7.register(editor, currentIndexState);
    return $_gapcrdipjew5etbo.get(editor, currentIndexState);
  });
  function Plugin () {
  }
