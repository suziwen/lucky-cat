...
var textpattern = (function () {
  ...
  var $_84glpmqhjew5evxp = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_2so6s2qpjew5evy3.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_2so6s2qpjew5evy3.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_2so6s2qpjew5evy3.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_9sc1xeqmjew5evy0.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_9sc1xeqmjew5evy0.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_9sc1xeqmjew5evy0.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !VK.modifierPressed(e)) {
        $_19qfx0qljew5evxs.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_19qfx0qljew5evxs.checkKeyCode(keyCodes, e)) {
        $_19qfx0qljew5evxs.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_19qfx0qljew5evxs.checkCharCode(charCodes, e)) {
  ...
  var $_9snio1qijew5evxq = { setup: setup };

  PluginManager.add('textpattern', function (editor) {
    var patternsState = Cell($_84glpmqhjew5evxp.getPatterns(editor.settings));
    $_9snio1qijew5evxq.setup(editor, patternsState);
    return $_7n3rw5qgjew5evxo.get(patternsState);
  });
  function Plugin () {
  }
