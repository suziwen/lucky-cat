...
(function( root, factory ) {
	...
	function initialize( options ) {
		...
		if( !features.transforms2d && !features.transforms3d ) {
			...
			for( var i = 0, len = images.length; i < len; i++ ) {
				var image = images[i];
				if( image.getAttribute( 'data-xsj-src' ) ) {
					image.setAttribute( 'src', image.getAttribute( 'data-xsj-src' ) );
					image.removeAttribute( 'data-xsj-src' );
				}
			}
	 ...
	 * @param {boolean} slient for trigger slidechangedEvent
	 */
	function slide( h, v, f, o, slient ) {
	 ...
	 * that is set to load lazily (data-xsj-src).
	 */
	function showSlide( slide ) {
		...
		// Media elements with data-xsj-src attributes
		toArray( slide.querySelectorAll( 'img[data-xsj-src], video[data-xsj-src], audio[data-xsj-src], iframe[data-xsj-src]' ) ).forEach( function( element ) {
			element.setAttribute( 'src', element.getAttribute( 'data-xsj-src' ) );
			element.removeAttribute( 'data-xsj-src' );
		} );
		...
		toArray( slide.querySelectorAll( 'video, audio' ) ).forEach( function( media ) {
			...
			toArray( media.querySelectorAll( 'source[data-xsj-src]' ) ).forEach( function( source ) {
				source.setAttribute( 'src', source.getAttribute( 'data-xsj-src' ) );
				source.removeAttribute( 'data-xsj-src' );
				sources += 1;
			} );
