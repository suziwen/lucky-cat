...
var MarkdownHighlightRules = function() {
    ...
    this.addRules({
        "basic" : [{
            ...
            token : ["text", "constant", "text", "url.link", "string", "text"],
            regex : "^([ ]{0,3}\\[)([^\\]]+)(\\]:\\s*)([^ ]+)(\\s*(?:[\"][^\"]+[\"])?(\\s*))$"
        }, { // link by reference
...
(function() {
    ...
    this.transformAction = function(state, action, editor, session, param){
      if (action == 'deletion' && (state === 'listblock-start' || state === 'listblock' || state === 'blockquote')) {
        var chars = param.end.column - param.start.column;
        if ( param.start.row === param.end.row && chars === 1){
          var textRange = new Range(param.start.row, 0, param.end.row, param.end.column);
          var textRangeStr = session.getTextRange(textRange);
          var match = /^(\s*)([-+*>]\s$|\d+\.\s$)/.exec(textRangeStr);
          if (!!match) {
            var blankChar = [];
            for(var i =0 ; i < match[2].length; i++) {
              blankChar.push(" ")
            }
            var replacedRange = new Range(param.start.row, param.end.column - match[2].length, param.end.row, param.end.column)
            session.replace(replacedRange, blankChar.join(""))
            return new Range(param.start.row, param.end.column, param.end.row, param.end.column)
          }
        }
        return false
      }
      if (action == 'insertion' && (state === 'listblock' || state === 'listblock-start' || state === 'blockquote') && param === '\n'){
        var cursor = editor.getCursorPosition();
        var line = session.doc.getLine(cursor.row);
        var match = /^(\s*)(?:([-+*>])|(\d+)\.)(\s*)$/.exec(line);
        if (!!match) {
          var range = editor.getSelectionRange();
          range.start.column = range.start.column - line.length;
          //var range = new Range(cursor.row, cursor.column, cursor.row, (cursor.column - line.length))
      ...
      var e = window.event;
      var curOp = editor.curOp;
      var _whiteRe = /[\s]{1,3}/
      if (curOp && action == 'insertion' && (state === 'listblock' || state === 'listblock-start' || state === 'blockquote') && _whiteRe.test(param) && curOp.command.name === 'indent'){
        var cursor = editor.getCursorPosition();
        var paramLength = param.length;
        var line = session.doc.getLine(cursor.row);
        var range = editor.getSelectionRange();
        range.start.column = 0;
        var prefixText = session.getTextRange(range);
        var match = /^(\s*)(?:([-+*>])|(\d+)\.)(\s*)$/.exec(prefixText);
        if (!!match) {
          var range = editor.getSelectionRange();
          range.start.column = 0;
          ...
          //var range = new Range(cursor.row, cursor.column, cursor.row, (cursor.column - line.length))
          session.remove(range);
          var orignColumn = cursor.column;
          cursor.column = 0;
          var insertedText = '    ' + line;
          session.insert(cursor, insertedText);
          range.start.column = orignColumn;
          range.end.column = insertedText.length;
          session.remove(range);
          editor.clearSelection();
          range.end.column = 0;
          return {
            selection: [4,4],
            text: insertedText.substring(orignColumn)
          }
        }
      }
    }
    ...
    this.getNextLineIndent = function(state, line, tab) {
        if (state == "listblock" || state == "blockquote") {
            var match = /^(\s*)(?:([-+*>])|(\d+)\.)(\s+)/.exec(line);
            if (!match)
                return this.$getIndent(line);
            var marker = match[2];
