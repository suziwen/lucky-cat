...
(function(cledit) {
    ...
    var action = function(action, options) {
        ...
        editor.setContent(state.before + state.selection + state.after);
        selectionMgr.setSelectionStartEnd(state.selectionStart, state.selectionEnd);
        selectionMgr.restoreSelection();
    };

//    var indentRegex = /^ {0,3}>[ ]*|^[ \t]*(?:[*+\-]|(\d+)\.|)[ \t]|^\s+/;
    var indentRegex =  /^[ \t]*(?:[*+\-]|(\d+)\.)[ \t]*(\[[\sxX]\])?[ \t]?|^ {0,3}>[ ]*|^[ \t]*(?:[*+\-]|(\d+)\.)[ \t]|^\s+/;
    var actions = {
        indent: function(state, options) {
            var editor = options.editor;
            var selectionMgr = editor.selectionMgr;
            var undoMgr = editor.undoMgr;
